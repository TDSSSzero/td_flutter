...
class _AutoSizeTextFieldState extends State<AutoSizeTextField> {
  ...
  List _calculateFontSize(BoxConstraints size, TextStyle? style, int? maxLines) {
    ...
    if (presetFontSizes == null) {
      ...
      if (_checkTextFitsCustom(span, defaultScale, maxLines, size)) {
    ...
    while (left <= right) {
      ...
      if (_checkTextFitsCustom(span, scale, maxLines, size)) {
        left = mid + 1;
        lastValueFits = true;
      } else {
        right = mid - 1;
  ...
  ///同原auto_size_text，当达到最大行数时，才缩小字体
  bool _checkTextFitsCustom(
